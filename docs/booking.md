...
- Loading UI (B3d-2; spinner ruling 2026-07-29): the App Router `loading.tsx` route loaders for /book and /account render a centered champagne spinner during the live server fetch, so navigation shows a page-level loader instead of a blank or frozen previous page. The spinner is a thin rotating arc over a faint full-circle track in the global accent, gated `motion-safe` so reduced motion holds it static as a partial ring. This supersedes the earlier quiet-line-and-hairline-pulse route loader and the no-spinner rule for route loaders; the checkout surfaces (§12.11) keep their eyebrow and copy. The /book availability grid still uses its own pulse-placeholder skeleton and the account list region keeps its pulse rows (§13.4); those skeletons are not spinners.
...
First Name and Last Name render as two columns on one row at the form's full width (they are two halves of one name); the row collapses to stacked single-column below 560px. Email, Password, and Phone remain full-width single-column. This is the one two-column exception to the narrow-block rule, on both sign-up and the profile section.
...
The amendment rests on a read-only probe of a fresh app-created account (2026-07-21): its metadata holds `first_name`, `last_name`, and `display_name`, and `display_name` is exactly the two parts joined by one space. `display_name` is therefore derived by the app, not an independent truth, so the earlier single-field model would have left the parts stale after any web edit. Writing all three keys means the app reads correctly whichever key it uses, which also settles the one question the probe could not answer: whether the app reads the parts or the composed value. Outstanding with the vendor, as a safety net only and blocking nothing: whether any name key beyond these three is read. Metadata only: the Supabase top-level phone identity field is never written.

/account gains a Profile section beneath the Email FactRow: a tracked "Profile" microlabel, First Name, Last Name, and Phone Fields prefilled from user metadata (`first_name`, `last_name`, `phone`), and a solid Save button. Prefill reads the name parts, never the derived `display_name`, so nothing has to guess at splitting a name. Both parts are required on save, phone optional (trimmed, no format enforcement). Saving runs a Server Action calling `updateUser` with the same three name keys as sign up, plus `phone` when given; an emptied Phone sends `null`, which drops the key rather than leaving an empty string, so a wrong number can be cleared. `updateUser` merges into `user_metadata`, so the app's own keys (`locale`, `marketing_opt_in`, `terms_accepted_at`, and the rest) survive untouched. Success renders the §9.7 confirmation line in place, errors reuse the §9.7 fallback; a missing part routes the §9.7 name error to the field at fault. Because the form writes every key the app might read, an app-created account opens with its name already filled and a web edit is visible in the app immediately. Email is identity and stays read only.

Migration: an account created by the earlier single-field build carries `display_name` with no parts. Its Profile form opens with empty name fields and heals on the first save. Nothing backfills existing accounts, in code or by hand.
...
- Field labels: `First Name` · `Last Name` · `Phone` with hint `Optional.`
- Name errors (sign up and profile), each routed to its own field: first name missing `Please enter your first name.` · last name missing `Please enter your last name.`
